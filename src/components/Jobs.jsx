...
const Jobs = () => {
  ...
  // type of Object
  if (job && !Array.isArray(job)) jobs.push(job);
  // console.log(jobs)
  // [
  //     { company: "Olla", jobs: [] },
              ...
              {showCard && (
                <Card>
                  <div className="pb-4">
                    ...
                    <div style={{ fontSize: "16px", fontWeight: "600", color: "#0A66C2" }} className="mb-1 ms-3">
                      {selectedJob.title}
                    </div>
                    <div className="ms-3 my-2" style={{ fontSize: "14px" }}>
                      {selectedJob.company_name}
                    </div>
                    <div className="my-1 ms-3" style={{ fontSize: "13px", color: "grey" }}>
                      {selectedJob.candidate_required_location} ({selectedJob.job_type})
                    </div>
                    <div className="ms-3 my-2" style={{ fontSize: "14px" }}>
                    ...
                    <a href={selectedJob.url} className="pb-3 ms-3" style={{ color: "#0A66C2", marginBottom: "10", textDecoration: "none" }}>
            ...
            <div>
              {jobs.map((job) => (
                <Card key={job.company} className="my-3">
                  ...
                  <div className="ms-3 mt-1" style={{ color: "grey", fontSize: "14px" }}>
                    Esplora le offerte di lavoro rilevanti nella tua rete
                  </div>
                  {job.jobs?.map((j) => (
                    <div key={j._id} className="d-flex justify-content-between border-bottom">
                      <div className="ms-3 mt-2 ">
                        <div style={{ fontSize: "16px", fontWeight: "600", color: "#0A66C2" }} className="mb-1">
                          {j.title}
                        </div>
                        <div style={{ fontSize: "14px" }}>{j.company_name}</div>
                        <div className="my-1  pb-4" style={{ fontSize: "13px", color: "grey" }}>
                          {j.candidate_required_location} ({j.job_type})
                        </div>
                      </div>
                      <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-x-lg me-4 mt-4" viewBox="0 0 16 16">
                          <path d="M2.146 2.854a.5.5 0 1 1 .708-.708L8 7.293l5.146-5.147a.5.5 0 0 1 .708.708L8.707 8l5.147 5.146a.5.5 0 0 1-.708.708L8 8.707l-5.146 5.147a.5.5 0 0 1-.708-.708L7.293 8z" />
                        </svg>
                      </div>
                    </div>
